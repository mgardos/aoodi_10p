...
//		return numerador.por(sumandoAsFraccion.denominador()).mas(denominador.por(sumandoAsFraccion.numerador()))
//				.dividido(denominador.por(sumandoAsFraccion.denominador()));
	}

    protected Numero sumarEntero(Entero entero) {
        int mcm = entero.value * denominador.value;
        int primerSumando = mcm * entero.value;
        int segundoSumando = (mcm / denominador.value) * numerador.value;

        int numerador = primerSumando + segundoSumando;

        return Fraccion.dividir( new Entero(numerador), new Entero(mcm) );
    }

    protected Numero sumarFraccion(Fraccion fraccion) {
        return fraccion.mas(this);
    }
	...
	public Numero dividido(Numero divisor) {
		...
		return numerador.por(divisorAsFraccion.denominador()).dividido(denominador.por(divisorAsFraccion.numerador()));
	}
}
